...
ScopedSearch::RSpec::Database.test_databases.each do |db|
  ...
  describe ScopedSearch, "using a #{db} database" do
    ...
    context "when the model does not have searchable string columns" do
      before(:all) do
        ScopedSearch::RSpec::Database.establish_named_connection(db)

        @class = ScopedSearch::RSpec::Database.create_model(name: :string, age: :integer) do |klass|
          klass.scoped_search :on => :age
        end

        @class.create!(name: 'Jane', age: 23)
        @class.create!(name: 'John', age: 42)
      end
      ...
      it 'should return records when the query matches the column types pattern' do
        @class.search_for('42').count.should == 1
      end

      it 'should not return records when the column type and the query value cannot match' do
        @class.search_for('John').count.should == 0
      end
    end

    context "when the model has searchable string columns" do
      before(:all) do
        ScopedSearch::RSpec::Database.establish_named_connection(db)
